...
class GraphRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(self, query: str, **kwargs) -> List[Document]:
        ...
        if not vector_results:
            print("No vector results, returning empty")
            return []
        ...
        print(f"[Step 5] Total docs before rerank: {len(all_docs)} (vector={len(page_docs)}, graph={len(graph_docs)}, community={len(community_pages)})")
